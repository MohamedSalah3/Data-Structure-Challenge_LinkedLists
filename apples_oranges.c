...
{
int loop=0,apples_counter = 0,oranges_counter = 0;
for(loop=0;loop<apples_count;loop++)
{
apples[loop]=apples[loop]+a;	
	
}

for(loop=0;loop<apples_count;loop++)
{
if(apples[loop] >= s &&apples[loop] <= t){
	apples_counter++;
}
	
}

for(loop=0;loop<oranges_count;loop++)
{
oranges[loop]=oranges[loop] + b;		
}

for(loop=0;loop<oranges_count;loop++)
{
if(oranges[loop] >= s &&oranges[loop] <= t){
	oranges_counter++;
}
}
printf("%d\n%d",apples_counter,oranges_counter);
